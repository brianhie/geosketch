...
def process_h5(fname, min_trans=MIN_TRANSCRIPTS, genome='mm10'):
    X, genes = load_h5(fname, genome=genome)
...
def process(data_names, min_trans=MIN_TRANSCRIPTS):
    for data_name in data_names:
        ...
        if os.path.isdir(name):
            ...
            process_h5(name, min_trans=min_trans, genome=data_name.split()[1])
        elif os.path.isfile(name + '.h5'):
            process_h5(name + '.h5', min_trans=min_trans,
                       genome=data_name.split()[1])
